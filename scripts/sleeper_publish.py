...
# keep old name working in case it appears anywhere
def now_iso():
    return now_utc()
...
def resolve_season_week(default_season=2025, default_week=6) -> tuple[int,int]:
    """Use env if provided; otherwise detect live season/week from Sleeper."""
...
# ---------- API wrappers ----------
def get_users():     return http_get(f"{BASE}/league/{LEAGUE_ID}/users", default=[]) or []
def get_rosters():   return http_get(f"{BASE}/league/{LEAGUE_ID}/rosters", default=[]) or []
def get_matchups(w): return http_get(f"{BASE}/league/{LEAGUE_ID}/matchups/{w}", default=[]) or []
def get_tx(w):       return http_get(f"{BASE}/league/{LEAGUE_ID}/transactions/{w}", default=[]) or []
...
def get_players_catalog() -> Dict[str, Any]:
    ...
    data = http_get(f"{BASE}/players/nfl", default={}) or {}
...
def get_trending(h, n):
    ...
    return {"generated_at": now_utc(), "lookback_hours": h, "limit": n, "adds": adds or [], "drops": drops or []}

# ---------- builders ----------
def decorate(pid: str, catalog: Dict[str, Any]) -> Dict[str, Any]:
...
def build_rosters(rosters, users, catalog):
    ...
    payload = {"season": SEASON, "generated_at": now_utc(), "league_id": LEAGUE_ID, "rosters": out}
...
def build_standings(rosters_payload):
    ...
    for r in rosters_payload["rosters"]:
        ...
        rows.append({
            "roster_id": r["roster_id"], "owner": r.get("owner"), "team_name": r.get("team_name"),
            "wins": w, "losses": l, "ties": t, "win_pct": (w + 0.5*t) / max(1, (w+l+t)), "points_for": pf
        })
    rows.sort(key=lambda x: (-x["wins"], -x["win_pct"], -x["points_for"]))
    return {"season": SEASON, "generated_at": now_utc(), "league_id": LEAGUE_ID, "standings": rows}
...
def build_matchups(week, matchups, rosters, users):
    ...
    def owner_name(rid):
        uid = (roster_by_id.get(rid) or {}).get("owner_id")
        u = users_by_id.get(uid, {})
        return (u.get("metadata") or {}).get("team_name") or u.get("display_name") or u.get("username")
    games = {}
    for m in matchups or []:
        mid = m.get("matchup_id") if m.get("matchup_id") is not None else 0
    ...
    return {
        "season": SEASON, "week": week, "generated_at": now_utc(), "league_id": LEAGUE_ID,
        "games": [{"matchup_id": k, "teams": v} for k, v in sorted(games.items(), key=lambda kv: kv[0])]
    }

def build_transactions(week, moves):
    return {"season": SEASON, "week": week, "generated_at": now_utc(), "league_id": LEAGUE_ID, "moves": moves or []}
...
def build_available(catalog, rostered_ids: Set[str]):
    ...
    for pid, p in catalog.items():
        ...
        if pos in {"QB","RB","WR","TE","K","DEF"} and pid not in rostered_ids:
            available.append({
                "player_id": pid, "full_name": p.get("full_name") or p.get("name"),
                "team": p.get("team"), "position": pos, "status": p.get("status")
            })
    available.sort(key=lambda x: (x["position"] or "ZZ", x["full_name"] or ""))
    return {"season": SEASON, "generated_at": now_utc(), "league_id": LEAGUE_ID, "count": len(available), "players": available}
...
def build_injuries(catalog, rostered_ids: Set[str]):
    ...
    for pid in rostered_ids:
        ...
        if p.get("injury_status"):
            injured.append({
                "player_id": str(pid), "full_name": p.get("full_name") or p.get("name"),
                "team": p.get("team"), "position": p.get("position"),
                "injury_status": p.get("injury_status"), "injury_body_part": p.get("injury_body_part"),
                "injury_start_date": p.get("injury_start_date"), "news_updated": p.get("news_updated"),
            })
    injured.sort(key=lambda x: (x["position"] or "ZZ", x["full_name"] or ""))
    return {"season": SEASON, "generated_at": now_utc(), "league_id": LEAGUE_ID, "players": injured}
...
def main():
    ...
    write_json(f"trending/{dt.datetime.utcnow():%Y-%m-%d}.json", get_trending(LOOKBACK_H, TREND_LIMIT))
    ...
    # meta index so the web UI/agent can discover season/week and freshness
    meta = {
        "season": SEASON,
        "latest_week": WEEK,
        "updated_at": now_utc(),
